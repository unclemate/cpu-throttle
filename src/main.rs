...
///
/// # Design Rationale (SOLID-S, DRY, KISS)
/// - Single Responsibility: Only handles log file opening logic
/// - Reusability: Can be used in tests with custom paths
/// - Simplicity: Truncates on startup, no complex rotation
fn open_log_file(path: &str) -> Result<fs::File> {
    OpenOptions::new()
        .write(true)
        .create(true)
        .truncate(true)
...
mod tests {
    ...
    // Log file tests (KISS: truncate on startup)
    #[test]
    fn test_open_log_file_creates_new_file() {
        let temp_path = "/tmp/test_log_new.log";
        // Ensure file doesn't exist
        let _ = fs::remove_file(temp_path);

        let file = open_log_file(temp_path);
        assert!(file.is_ok(), "Should create new log file");

        // Verify file exists
        assert!(Path::new(temp_path).exists(), "Log file should exist after open");
    ...
    #[test]
    fn test_open_log_file_truncates_existing() {
        let temp_path = "/tmp/test_log_truncate.log";

        // Create file with some content
        fs::write(temp_path, "old log content\nline 2\nline 3\n").unwrap();
        let metadata_before = fs::metadata(temp_path).unwrap();
        let size_before = metadata_before.len();
        assert!(size_before > 0, "File should have content before truncate");

        // Open with open_log_file (should truncate)
        let file = open_log_file(temp_path);
        assert!(file.is_ok(), "Should open existing log file");

        // Verify file is now empty
        let metadata_after = fs::metadata(temp_path).unwrap();
        let size_after = metadata_after.len();
        assert_eq!(size_after, 0, "File should be empty after truncate (KISS: clean on startup)");
    ...
    #[test]
    fn test_open_log_file_is_writable() {
        let temp_path = "/tmp/test_log_writable.log";
        let _ = fs::remove_file(temp_path);

        let mut file = open_log_file(temp_path).unwrap();

        // Write test data
        use std::io::Write;
        let test_content = "test log entry\n";
        file.write_all(test_content.as_bytes()).unwrap();
        file.flush().unwrap();

        // Verify content was written
        let content = fs::read_to_string(temp_path).unwrap();
        assert_eq!(content, test_content, "Should be able to write to log file");
